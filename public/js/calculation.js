...
document.addEventListener("DOMContentLoaded", () => {
    ...
    orderButtons.forEach(button =>
        button.addEventListener('click', () => {
            const serviceName = button.getAttribute('data-service');
            const priceElement = button.closest('.calculation-area, .calculation-kaliningrad')
                ?.querySelector('span[id^="total-"]');
            const priceValue = priceElement ? parseFloat(priceElement.textContent) : 0;

            serviceInput.value = serviceName; // Set service name
            document.getElementById('price-input').value = priceValue.toFixed(2);
            popup.classList.remove('hidden');
        })
    );
